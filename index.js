...
wss.on("connection", (twilioSocket) => {
  ...
  // ✅ FAST STOP (minimal): cancel immediately on speech_started, but with grace window.
  let preCancelFired = false;
  let aiSpeechStartedAt = 0;
  const BARGE_IN_GRACE_MS = 280; // tune 220–350
    ...
    sendToOpenAI({
      ...
      session: {
        ...
        turn_detection: {
          ...
          silence_duration_ms: 450 // ✅ faster transcript finalization
        },
  ...
  openaiSocket.on("message", (raw) => {
    ...
    if (evt.type === "response.audio.started") {
      isAISpeaking = true;
      aiSpeechStartedAt = Date.now(); // ✅ for grace window
      // reset so next user interruption can cancel
      preCancelFired = false;
    }
    ...
    // ✅ FAST STOP: cancel immediately on caller speech start (after grace window)
    if (evt.type === "input_audio_buffer.speech_started") {
      if (isAISpeaking || responseInFlight) {
        ...
        if (!preCancelFired && (Date.now() - aiSpeechStartedAt) > BARGE_IN_GRACE_MS) {
          ...
          // Wait for transcript to decide what to do (filler vs real question)
        }
      }
    }
    ...
    if (evt.type === "conversation.item.input_audio_transcription.completed") {
      ...
      // If we already pre-canceled (Roy stopped instantly), now decide how to respond.
      if (preCancelFired) {
        pendingBargeIn = false;
        preCancelFired = false;

        // If it was filler, keep it minimal.
        if (filler) {
      ...
      // Normal path: Roy not talking -> respond normally
      pendingBargeIn = false;
      preCancelFired = false;
      injectUserTextAndRespond(transcript);
    }

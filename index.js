...
- Maintain a confident, friendly, and relaxed tone. Do not sound robotic or overly formal.

## Listening and Interruptions
- When the caller says filler words while you are speaking, do not pause—continue naturally.
- Only stop mid-sentence if the caller clearly asks a question.

## Scope of Service
- Explain 24/7 AI covers calls, bookings, reservations, lead capture, and onboarding.
`.trim();
...
// NEW: require a “real” question (filters tiny echo fragments like "what", "how", etc.)
function isStrongQuestion(text) {
  ...
  // If it's too short, it's almost always echo/garbage
  if (w.length < 3 && cleanedLen < 12) return false;

  // Otherwise, use the normal heuristic
...
wss.on("connection", (twilioSocket) => {
  ...
  // NEW: only allow cancel when we truly detected caller speech during Roy speech
  ...
  openaiSocket.on("open", () => {
    ...
    // Minimal: enable VAD + transcription
    sendToOpenAI({
      type: "session.update",
      session: {
  ...
  openaiSocket.on("message", (raw) => {
    ...
    // NEW: only mark pending barge-in when caller speech starts DURING AI speech
    if (evt.type === "input_audio_buffer.speech_started") {
      if (isAISpeaking || responseInFlight) pendingBargeIn = true;
    }

    // Commit on caller speech stop
    ...
    // Decide whether to cancel (only on REAL question + real barge-in)
    if (evt.type === "conversation.item.input_audio_transcription.completed") {
      ...
      // If caller spoke while Roy was talking, ONLY cancel if (strong question && not filler)
      if ((isAISpeaking || responseInFlight) && pendingBargeIn) {
        if (!filler && strongQ) {
          // Cancel Roy and answer
        ...
        // Not a real question -> ignore, let Roy continue
        pendingBargeIn = false;
        return;
      }

      // Normal case: Roy not speaking -> answer normally
      pendingBargeIn = false;
      sendToOpenAI({ type: "response.create" });
    }
  });
  ...
  const isCallerAudio = (track) => {
    if (!track) return false; // keeping your base behavior

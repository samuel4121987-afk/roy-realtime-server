...
wss.on("connection", (twilioSocket) => {
  ...
  openaiSocket.on("message", (raw) => {
    ...
    if (evt.type === "conversation.item.input_audio_transcription.completed") {
      ...
      // ✅ IMPORTANT FIX:
      // If Roy is talking and a real question transcript arrives, STOP and answer
      // EVEN IF pendingBargeIn was not set (VAD timing sometimes misses speech_started).
      ...
      if (aiTalking && pendingBargeIn) {
        // Only cancel if it's a REAL question (and not filler) — already handled above,
        // so at this point it’s not a real question -> ignore
        pendingBargeIn = false;
        return;
      }

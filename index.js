...
wss.on("connection", (twilioSocket) => {
  ...
  openaiSocket.on("message", (raw) => {
    ...
    // Handle incoming transcripts - if Roy is speaking, STOP HIM IMMEDIATELY
    if (evt.type === "conversation.item.input_audio_transcription.completed") {
      const transcript = evt.transcript || "";
      if (transcript.trim()) {
        console.log("📝 Transcript:", transcript);
        
        // If Roy is currently speaking, CANCEL and answer the question
        if (isRoySpeaking) {
          console.log("🛑 Interrupting Roy to answer question");

...
wss.on("connection", (twilioSocket) => {
  ...
  // TWO-PHASE BARGE-IN:
  // Phase 1: Stop Roy immediately when user audio overlaps
  ...
  const BARGE_GRACE_MS = 50; // grace period before counting packets
  const PRE_CANCEL_PACKETS = 3; // packets needed after grace to trigger stop (~60ms total)
  ...
  openaiSocket.on("message", (raw) => {
    ...
    // PHASE 1: Mark when user starts speaking while Roy is talking
    if (evt.type === "input_audio_buffer.speech_started") {
      if (isAISpeaking || responseInFlight) {
        bargeInProgress = true;
        bargePacketCount = 0;
        bargeStartTime = Date.now();
      }
    }
  ...
  twilioSocket.on("message", (msg) => {
    ...
    if (data.event === "media") {
      ...
      // PHASE 1: Count packets after grace period, stop Roy if threshold reached
      if (bargeInProgress && (isAISpeaking || responseInFlight)) {
        const elapsedMs = Date.now() - bargeStartTime;
        
        // After grace period, start counting packets
        if (elapsedMs >= BARGE_GRACE_MS) {
          bargePacketCount += 1;
          
          // If enough packets received, STOP Roy immediately
          if (bargePacketCount >= PRE_CANCEL_PACKETS) {
            console.log("⚡ BARGE-IN: Stopping Roy after", elapsedMs, "ms");
            cancelAndClearTwilio();
            // bargeInProgress stays true so Phase 2 knows this was a barge-in
          }
        }
      }

...
wss.on("connection", (twilioSocket) => {
  ...
  const MIN_BARGE_MS = 220;

  let bargeStartedAt = 0;
  let aiSpeechStartedAt = 0;
  const BARGE_IN_GRACE_MS = 320;

  // ✅ NEW: dedupe + queue to prevent “answered twice”
  let lastUserTranscriptNorm = "";
  let lastUserTranscriptAt = 0;
  const TRANSCRIPT_DEDUPE_MS = 1600;
  ...
  openaiSocket.on("message", (raw) => {
    ...
    if (evt.type === "response.done") {
      responseInFlight = false;
      isAISpeaking = false;
      // ✅ after any response fully ends, process a queued user transcript (if any)
    ...
    if (evt.type === "response.audio.done") {
      isAISpeaking = false;
      // ✅ also try here (sometimes audio.done arrives before response.done)
      maybeProcessQueuedTranscript();
    }
    ...
    if (evt.type === "conversation.item.input_audio_transcription.completed") {
      ...
      // ✅ DEDUPE: prevents “Roy answered twice”
      ...
      if (preCancelFired) {
        ...
        // If a response is already being generated, queue it to avoid double responses
      ...
      if ((isAISpeaking || responseInFlight) && pendingBargeIn) {
        if (!filler && strongQ) {
          ...
          // If cancellation causes OpenAI to still be mid-cycle, queue it safely
          if (responseInFlight || isAISpeaking) {
            queuedTranscript = transcript;
            return;
          }
      ...
      // Roy not talking: but if a response is already in flight, queue to prevent double
      pendingBargeIn = false;
      preCancelFired = false;
